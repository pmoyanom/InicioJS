...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    botonAleatorio.id = 'boton-aleatorio';
    botonAleatorio.textContent = 'Elegir Pokémon Aleatorio';

    // Crear contenedor para los botones y colocarlo al inicio
    const contenedorBotones = document.createElement('div');
    contenedorBotones.classList.add('contenedor-botones');
    contenedorBotones.appendChild(botonSiguiente);
    contenedorBotones.appendChild(botonAleatorio);
    if (document.body.firstChild) {
        document.body.insertBefore(contenedorBotones, document.body.firstChild);
    } else {
        document.body.appendChild(contenedorBotones);
    }

    let pokemonSeleccionado = null;
    let pokemonCardSeleccionada = null;
    let pokemonOponente = null;

    // Generar la lista de Pokémon en la interfaz
    function generarListaPokemon() {
        if (listaPokemonElement) {
            pokemonList.forEach((pokemon, index) => {
                const pokemonCard = document.createElement('div');
                pokemonCard.classList.add('pokemon-card');

                // Determinar si el Pokémon es "shiny" con una probabilidad del 1%
                const isShiny = Math.random() < 0.1; // 1% de probabilidad
                let pokemonImg = pokemon.img;

                if (isShiny) {
                    pokemonImg = pokemonImg.replace('normal', 'shiny');
                }

                // Asignar color de fondo según el tipo de Pokémon
                if (pokemon.types.length === 1) {
                    pokemonCard.style.backgroundColor = typeColors[pokemon.types[0]];
                } else {
                    const color1 = typeColors[pokemon.types[0]];
                    const color2 = typeColors[pokemon.types[1]];
                    pokemonCard.style.background = `linear-gradient(135deg, ${color1}, ${color2})`;
                }

                // Crear HTML de la tarjeta del Pokémon
                pokemonCard.innerHTML = `
                    <img src="${pokemonImg}" alt="${pokemon.name}">
                    <h3>${pokemon.name}</h3>
                    <p>Tipos: ${pokemon.types.join(', ')}</p>
                    <button onclick="seleccionarPokemon(${index})">Seleccionar</button>
                `;

                // Añadir estrella si el Pokémon es shiny
                if (isShiny) {
                    const shinyStar = document.createElement('img');
                    shinyStar.src = 'https://res.cloudinary.com/dq8kckqtc/image/upload/v1732473453/images__1_-removebg-preview_cxee3p.png'; // Asegúrate de tener esta imagen en tu carpeta de imágenes
                    shinyStar.alt = 'Shiny Star';
                    shinyStar.classList.add('shiny-star');
                    pokemonCard.appendChild(shinyStar);
                }

                listaPokemonElement.appendChild(pokemonCard);
            });
        }
    }
    ...
    // Seleccionar un Pokémon y resaltar su tarjeta
    window.seleccionarPokemon = function(index) {
        pokemonSeleccionado = pokemonList[index];
        botonSiguiente.disabled = false;

        // Resaltar la tarjeta del Pokémon seleccionado
        if (pokemonCardSeleccionada) {
            pokemonCardSeleccionada.classList.remove('seleccionado');
        }
        pokemonCardSeleccionada = listaPokemonElement.children[index];
        pokemonCardSeleccionada.classList.add('seleccionado');
    }

    // Elegir un Pokémon aleatorio y resaltar su tarjeta
    botonAleatorio.addEventListener('click', () => {
        const indiceAleatorio = Math.floor(Math.random() * pokemonList.length);
        pokemonSeleccionado = pokemonList[indiceAleatorio];
        botonSiguiente.disabled = false;
        ...
        pokemonCardSeleccionada = listaPokemonElement.children[indiceAleatorio];
        pokemonCardSeleccionada.classList.add('seleccionado');
    });

    // Elegir un Pokémon oponente aleatorio
    function elegirPokemonOponente() {
        let indiceAleatorio;
        do {
            indiceAleatorio = Math.floor(Math.random() * pokemonList.length);
        } while (pokemonSeleccionado && pokemonList[indiceAleatorio].id === pokemonSeleccionado.id);
        pokemonOponente = pokemonList[indiceAleatorio];
    }

    // Evento para pasar a la siguiente pantalla
    botonSiguiente.addEventListener('click', () => {
        if (pokemonSeleccionado) {
            elegirPokemonOponente();
            // Guardar el Pokémon del jugador y el oponente en localStorage
            localStorage.setItem('pokemonJugador', JSON.stringify(pokemonSeleccionado));
            localStorage.setItem('pokemonOponente', JSON.stringify(pokemonOponente));
            // Redirigir a fase2.html
