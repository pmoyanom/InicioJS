const pokemons = [
    ...
    { id: 29, name: "Nidoran_f", img: "https://projectpokemon.org/images/normal-sprite/nidoran_f.gif", types: ["Poison"], attack: 47, defense: 52, hp: 55 },
    ...
    { id: 32, name: "Nidoran_m", img: "https://projectpokemon.org/images/normal-sprite/nidoran_m.gif", types: ["Poison"], attack: 57, defense: 40, hp: 46 },
    ...
    { id: 83, name: "Farfetchd", img: "https://projectpokemon.org/images/normal-sprite/farfetchd.gif", types: ["Normal", "Flying"], attack: 65, defense: 55, hp: 52 },
...
// Define color codes for each Pokémon type, allowing the UI to display type badges in specific colors
const TYPE_COLORS = {
    normal: '#A8A878',
    fire: '#F08030',
    water: '#6890F0',
...
// Variable to store the Pokémon selected by the user
let selectedPokemon = null;
// Reset any existing cookies for player and opponent Pokémon to start fresh
...
// Function to set a cookie with a specified name, value, and expiration in days (default: 7 days)
// This cookie saves a Pokémon selection to persist between sessions
...
// Parses the JSON stored in the cookie to restore Pokémon data
...
// Function that determines if a Pokémon is shiny, giving a 10% chance for shiny status
// A shiny Pokémon has a unique color and appearance
function isShiny() {
    return Math.random() < 0.1; 
}

// Function to generate the URL for the Pokémon's image, depending on its name and shiny status
// Uses "shiny-sprite" or "normal-sprite" based on the shiny condition
function getPokemonImage(pokemonName, isShinyPokemon) {
    const baseUrl = "https://projectpokemon.org/images/";
    const spriteType = isShinyPokemon ? "shiny-sprite" : "normal-sprite";
    
    let formattedName = pokemonName.toLowerCase();
    
    if(formattedName === "mrmime") {
        return `${baseUrl}${spriteType}/mr.mime.gif`;
    }
    
    return `${baseUrl}${spriteType}/${formattedName}.gif`;
}

// Function to create a badge element representing the Pokémon's type with a specific color and styling
// The badge is styled with colors from TYPE_COLORS and added to the Pokémon card
function createTypeBadge(type) {
    const badge = document.createElement('span');
    badge.textContent = type;
    badge.style.backgroundColor = TYPE_COLORS[type.toLowerCase()];
...
// Function to create a card element for each Pokémon, containing its image, name, and type badges
// The card includes an event listener for selection functionality
function createPokemonCard(pokemon, isShinyPokemon) {
    ...
    // Retrieve the Pokémon image based on its name and shiny status
    const imageUrl = getPokemonImage(pokemon.name.replace(/[♀♂\s.']/g, ''), isShinyPokemon);
    const image = document.createElement('img');
    image.src = imageUrl;
    image.alt = `${pokemon.name}${isShinyPokemon ? ' (Shiny)' : ''}`;
    
    // Display the Pokémon's name with a shiny star if applicable
    const name = document.createElement('h3');
    name.textContent = `${pokemon.name}${isShinyPokemon ? ' ⭐' : ''}`;
    
     // Container to hold type badges for each Pokémon type
    ...
    // Add a click event to the card for selecting this Pokémon
    card.addEventListener('click', () => {
        ...
        selectPokemon(selectedPokemonData);
    });

    return card;
}

// Function to select a random Pokémon from the list
// This is used to implement a "random Pokémon" feature
function getRandomPokemon() {
    const randomIndex = Math.floor(Math.random() * pokemons.length);
    return pokemons[randomIndex];
}

// Function to select a Pokémon and save it in a cookie to persist between sessions
// Also enables the "Next" button if it exists in the UI
function selectPokemon(pokemon) {
    selectedPokemon = pokemon;
    setCookie('playerPokemon', pokemon);
    ...
    highlightSelectedCard(pokemon.id);
}

// Function to highlight the selected Pokémon card visually
// This helps the user identify the currently selected Pokémon on the page
function highlightSelectedCard(pokemonId) {
    const allCards = document.querySelectorAll('.pokemon-card');
    allCards.forEach(card => {
        card.classList.remove('selected');
    });
    
    const selectedCard = document.querySelector(`.pokemon-card[data-pokemon-id="${pokemonId}"]`);
    if (selectedCard) {
...
// Initialize the page and Pokémon cards when the window loads
window.onload = () => {
    console.log("Window loaded"); // Debug
    const pokemonList = document.getElementById('pokemon-list');
   
    // Check if the Pokémon list container is available in the DOM
    if (!pokemonList) {
        console.error("Error: Contenedor de la lista de Pokémon no encontrado.");
        return;
    }

    const audio = document.getElementById("audio");
    if (audio) {
       // Enable audio on click if autoplay is restricted
        document.body.addEventListener('click', () => {
            audio.muted = false; // Desmutear el audio
            audio.play().catch(error => {
                console.log("Autoplay no permitido, el usuario debe interactuar primero.");
            });
        });
    }
    // Restore selected Pokémon from a previous session if available
    const savedPokemon = getCookie('playerPokemon');
    if (savedPokemon) {
        selectedPokemon = savedPokemon;
        const nextButton = document.getElementById('next-button');
        if (nextButton) {
            nextButton.disabled = false;
        }
    ...
    console.log("Creating Pokemon cards"); // Debug
     // Generate and display Pokémon cards from the list of available Pokémon
    pokemons.forEach(pokemon => {
        const pokemonIsShiny = isShiny();
        const card = createPokemonCard(pokemon, pokemonIsShiny);
        pokemonList.appendChild(card);
    });

     // Highlight the card of a previously selected Pokémon if it exists
    if (selectedPokemon) {
        highlightSelectedCard(selectedPokemon.id);
    }

    // Configure the "Next" button to redirect to the battle page if a Pokémon is selected
    const nextButton = document.getElementById('next-button');
    if (nextButton) {
        nextButton.disabled = !selectedPokemon;
    ...
    // Configure the "Random" button to select a random Pokémon and redirect to the battle page
    const randomButton = document.getElementById('random-button');
    if (randomButton) {
        randomButton.addEventListener('click', () => {
            const randomPokemon = getRandomPokemon();
            const pokemonIsShiny = isShiny();
            const imageUrl = getPokemonImage(randomPokemon.name.replace(/[♀♂\s.']/g, ''), pokemonIsShiny);
            const selectedRandomPokemon = {
                ...randomPokemon,
                isShiny: pokemonIsShiny,
                img: imageUrl
            };
            selectPokemon(selectedRandomPokemon);
            window.location.href = 'vs.html';
        });
    }
};
