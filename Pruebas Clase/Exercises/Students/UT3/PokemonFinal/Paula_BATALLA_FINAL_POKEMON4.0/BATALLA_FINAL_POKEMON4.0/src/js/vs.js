...
// Function to get and save a random opponent in a cookie each time the page loads
// Excludes the player’s Pokémon and chooses another randomly, with a 10% chance of being shiny
function getAndSaveRandomOpponent(playerPokemon) {
    // Filter the list of Pokémon to exclude the player’s Pokémon
    const availablePokemons = pokemons.filter(p => p.id !== playerPokemon.id);
    ...
    // Randomly determines if the opponent Pokémon will be shiny
    const isShinyOpponent = Math.random() < 0.1;

    // Creates the opponent Pokémon object with an image and shiny status based on the selection
    const opponentPokemon = {
        ...randomPokemon,
        isShiny: isShinyOpponent,
        img: getPokemonImage(randomPokemon.name.replace(/[♀♂\s.']/g, ''), isShinyOpponent)
    };

    // Save the opponent Pokémon data in a cookie
    setCookie('opponentPokemon', opponentPokemon);
...
// Function to retrieve a cookie by its name and return its value as a parsed JSON object
...
// Function to get the Pokémon image URL based on the Pokémon name and shiny status
...
// Function to create a type badge with a background color based on the Pokémon type
function createTypeBadge(type) {
    const badge = document.createElement('span');
    badge.textContent = type;
    badge.className = `pokemon-type-${type.toLowerCase()}`;
    return badge;
}

// Function to display the VS screen, showing player and opponent Pokémon with additional stats
function displayVSScreen(playerPokemon, opponentPokemon) {
    // Display player Pokémon details
    const playerContainer = document.getElementById('player-pokemon');
    if (playerContainer) {
        const playerImage = document.createElement('img');
        playerImage.src = playerPokemon.img;
        playerImage.alt = playerPokemon.name;
        playerContainer.appendChild(playerImage);

        const playerInfo = document.createElement('div');
        playerInfo.className = 'pokemon-info';
        
        const playerName = document.createElement('h3');
        playerName.textContent = `${playerPokemon.name}${playerPokemon.isShiny ? ' ⭐' : ''}`;
        playerInfo.appendChild(playerName);

        // Player Pokémon types
        const playerTypes = document.createElement('div');
        playerTypes.className = 'types-container';
        playerPokemon.types.forEach(type => {
            playerTypes.appendChild(createTypeBadge(type));
        });
        playerInfo.appendChild(playerTypes);

        // Display player Pokémon stats
        const playerStats = document.createElement('div');
        playerStats.className = 'pokemon-stats';
        playerStats.innerHTML = `
            <p><strong>HP:</strong> ${playerPokemon.hp}</p>
            <p><strong>Attack:</strong> ${playerPokemon.attack}</p>
            <p><strong>Defense:</strong> ${playerPokemon.defense}</p>
        `;
    ...
    if (opponentContainer) {
        ...
        opponentImage.src = opponentPokemon.img;
        opponentImage.alt = opponentPokemon.name;
        opponentContainer.appendChild(opponentImage);

        const opponentInfo = document.createElement('div');
        opponentInfo.className = 'pokemon-info';
        
        const opponentName = document.createElement('h3');
        opponentName.textContent = `${opponentPokemon.name}${opponentPokemon.isShiny ? ' ⭐' : ''}`;
        opponentInfo.appendChild(opponentName);

        // Opponent Pokémon types
        const opponentTypes = document.createElement('div');
        opponentTypes.className = 'types-container';
        opponentPokemon.types.forEach(type => {
            opponentTypes.appendChild(createTypeBadge(type));
        });
        opponentInfo.appendChild(opponentTypes);

        // Display opponent Pokémon stats
        const opponentStats = document.createElement('div');
        opponentStats.className = 'pokemon-stats';
        opponentStats.innerHTML = `
            <p><strong>HP:</strong> ${opponentPokemon.hp}</p>
            <p><strong>Attack:</strong> ${opponentPokemon.attack}</p>
            <p><strong>Defense:</strong> ${opponentPokemon.defense}</p>
        `;
        opponentInfo.appendChild(opponentStats);

        opponentContainer.appendChild(opponentInfo);
    }
}

// Page initialization when it loads
window.onload = () => {
    ...
    // Generate and save a new random opponent each time the VS screen loads
    const opponentPokemon = getAndSaveRandomOpponent(playerPokemon);

    // Display the player and opponent Pokémon on the VS screen
    displayVSScreen(playerPokemon, opponentPokemon);

    // Redirect to the battle screen when clicking the "Start Battle" button
